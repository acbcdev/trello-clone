...
export class DialogRowsComponent {
  ...
  name = signal<string>('')
  inputEditName = new FormControl('', { validators: [Validators.required, Validators.nullValidator] })
  descriptionInput = new FormControl('', { validators: [Validators.required, Validators.nullValidator] })
  ...
  changeNameHandler() {
    ...
    this.dataService.columns.update((prev) => prev.map((column) => (
      column.id === this.infoRow()?.ColumnId
        ? { ...column, items: column.items.map((item) => (item.id === this.infoRow()?.id ? { ...item, name: title ?? '' } : item)) }
        : column
    )))
  ...
  editDescription() {
    ...
    this.dataService.columns.update((prev) => prev.map((column) => (
      column.id === (this.infoRow()?.ColumnId ?? '')
        ? { ...column, items: column.items.map((item) => (item.id === (this.infoRow()?.id ?? '') ? { ...item, description: text ?? '' } : item)) }
        : column
    )))
